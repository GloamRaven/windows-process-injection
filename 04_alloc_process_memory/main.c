	...
	if (Process32FirstW(hSnap, &pe)) {
		do {
			if (wcscmp(procName, pe.szExeFile) == 0) {
				HANDLE hProc = OpenProcess(PROCESS_VM_OPERATION, FALSE, pe.th32ProcessID);
				if (hProc == NULL) {
					wprintf(L"OpenProcess failed. Error: %d\n", GetLastError());
				}
				else {
					LPVOID pMem = VirtualAllocEx(hProc, NULL, 4096, MEM_RESERVE | MEM_COMMIT, PAGE_READONLY);
					if (pMem == NULL) {
						wprintf(L"VirtualAllocEx failed. Error: %d\n", GetLastError());
					}
					else {
						wprintf(L"%ls(%d) allocated memory address: %p\n", pe.szExeFile, pe.th32ProcessID, pMem);
